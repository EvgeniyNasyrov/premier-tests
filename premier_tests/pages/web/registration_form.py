...
class RegistrationForm:
    def __init__(self):
        ...
        self.continue_button = s(
            (
                "xpath",
                '//button[@type="submit"] | //*[@data-testid="continue"] | //button[contains(.,"Далее") or contains(.,"Продолжить") or contains(.,"Войти")]',
            )
        )
        self.sliding_panel = s(
            (
                "xpath",
                '//*[contains(@class,"auth") or contains(@class,"modal") or contains(@class,"panel")] | //*[@data-testid="auth-panel"]',
            )
        )
        self.info_message = s(
            ("xpath", '//*[contains(@class,"error") or contains(@class,"invalid")] | //*[@data-testid="error"]')
        )
        ...
        if email_valid:
            ...
            try:
                ...
                try:
                    browser.element(
                        (
                            "xpath",
                            '//*[contains(@class,"error") or contains(text(),"Неверн") or contains(text(),"ошибк")]',
                        )
                    ).with_(timeout=2).should(be.visible)
                except (TimeoutException, WebDriverException):
            ...
            try:
                ...
                browser.element(
                    (
                        "xpath",
                        '//*[contains(text(),"Неверн") or contains(text(),"ошибк") or contains(text(),"invalid") or contains(text(),"Введите")]',
                    )
