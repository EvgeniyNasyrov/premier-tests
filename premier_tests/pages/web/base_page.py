...
def click_on_movie_preview():
    with step("Нажать на превью контента"):
        ...
        browser.element(
            (
                "xpath",
                '//a[.//img][contains(@href,"/") and not(starts-with(@href,"#"))] | //a[contains(@href,"/watch") or contains(@href,"/movie") or contains(@href,"/film") or contains(@href,"/p/")]',
            )
...
def click_on_promo():
    with step("Нажать на промо-блок"):
        browser.element(
            (
                "xpath",
                '//section[@data-qa-selector="promo-slider"]//a | //*[contains(@class,"w-promo")]//a | //button[contains(text(),"Подключить")]',
            )
        ).with_(timeout=6).should(be.visible).click()


def check_promo_title():
    with step("Проверить заголовок промо"):
        browser.element(
            ("xpath", '//*[contains(text(),"1₽") or contains(text(),"PREMIER") or contains(text(),"RUTUBE")]')
        ).should(be.visible)
...
def check_promo_button():
    with step("Проверить кнопку промо"):
        browser.element(
            ("xpath", '//button[contains(text(),"Подключить")] | //*[contains(text(),"Подключить")]')
        ).should(be.visible)
