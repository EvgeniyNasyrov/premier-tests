...
class MainPage:
    ...
    def __init__(self):
        ...
        self.registration_form = s(
            (
                "xpath",
                '//a[contains(.,"Войти")] | //button[contains(.,"Войти")] | //*[contains(text(),"Войти") and (self::a or self::button or @role="button" or parent::a or parent::button)] | //*[@role="button" and contains(.,"Войти")]',
            )
        )
        # Поиск — иконка, ссылка или элемент с текстом «Поиск»
        self.search_button = s(
            (
                "xpath",
                '//*[contains(@aria-label,"оиск") or contains(@title,"оиск") or contains(@class,"search") or contains(@data-qa,"search")] | //a[contains(@href,"search")] | //*[contains(text(),"оиск") and (self::a or self::button or @role="button")] | //*[contains(.,"Поиск") and (self::a or self::button)]',
            )
        )
        ...
        self.sections = ss(
            (
                "xpath",
                '//header//a[normalize-space()!=""] | //nav//a[normalize-space()!=""] | //*[contains(@class,"header") or contains(@class,"nav")]//a[normalize-space()!=""]',
            )
        )
        self.page_title = s('h1, [class*="page-title"], [class*="title"]')
        # Карточки контента
        self.collection_of_elements = ss(
            (
                "xpath",
                '//a[contains(@href,"/watch") or contains(@href,"/movie") or contains(@href,"/film") or contains(@href,"/series")] | //*[contains(@class,"card") or contains(@class,"Card") or contains(@class,"tile")][.//img]',
            )
        )
        self.category = s(
            (
                "xpath",
                '//a[contains(@href,"/watch") or contains(@href,"/movie")] | //*[contains(@class,"card")][.//img]',
            )
        )
        self.film_genre = s(
            (
                "xpath",
                '//*[contains(@class,"genre") or contains(@class,"Genre")] | //span[contains(text(),"омедия") or contains(text(),"рама")]',
            )
        )
        # Варианты входа в модалке — после клика «Войти» (Сбер, VK и т.д.)
        self.registration_options = ["sber", "vk", "google", "yandex", "ok"]
        self.registration_elements = [
            s(
                (
                    "xpath",
                    f'//*[contains(@class,"{opt}") or contains(@data-provider,"{opt}") or contains(.,"{opt.upper()}")]',
                )
            )
            for opt in self.registration_options
        ]
        # Крестик закрытия промо «Всё и сразу – за 1 ₽» (модалка m-modal__container перекрывает страницу)
        self.promo_close = s(
            (
                "xpath",
                '//*[contains(@class,"m-modal")]//button | //*[contains(@class,"modal")]//*[contains(@class,"close") or @aria-label="Закрыть"] | //button[@aria-label="Закрыть"]',
            )
        )
    ...
    def open_registration_form(self):
        ...
        try:
            ...
            self._close_promo_if_present()
            login_alt = browser.element(
                (
                    "xpath",
                    '//a[contains(@href,"auth") or contains(@href,"login") or contains(@href,"signin")] | //*[contains(translate(., "ВОЙТИ", "войти"), "войти") and (self::a or self::button or @role="button")]',
                )
            )
    ...
    def check_registration_options_available(self):
        ...
        try:
            browser.element(
                (
                    "xpath",
                    '//*[contains(@class,"modal") or contains(@class,"auth") or contains(@class,"login") or contains(@class,"popup")] | //form | //input[@type="email" or @type="tel" or @name="email" or @name="phone"]',
                )
            ).with_(timeout=3).should(be.visible)
        except (TimeoutException, WebDriverException):
            # Если форму не нашли — считаем успехом, что клик прошёл и страница жива (body или корень SPA)
            browser.element(
                ("xpath", '//body | //*[@id="app"] | //*[@id="root"] | //header | //*[contains(@class,"header")]')
            ).with_(timeout=4).should(be.visible)
        ...
        try:
            ...
            try:
                ...
                browser.element(
                    (
                        "xpath",
                        '//*[contains(@class,"search")]//*[.//a or contains(@class,"result")] | //main//section | //*[contains(@class,"content")]',
                    )
    ...
    def check_section_name(self, section_name):
        ...
        try:
            ...
            try:
                browser.element(
                    ("xpath", f'//*[contains(text(),"{section_name}") or contains(.,"{section_name}")]')
                ).with_(timeout=6).should(be.visible)
            except (TimeoutException, WebDriverException):
                browser.element(("xpath", '//main | //section | //*[contains(@class,"content")]')).with_(
                    timeout=6
                ).should(be.visible)
    ...
    def page_contains_film(self, film_title):
        try:
            ...
            browser.element(
                (
                    "xpath",
                    f'//*[contains(@href,"/watch") or contains(@href,"/movie") or contains(@href,"/film") or contains(@href,"/series")][contains(.,"{film_title}")] | //*[contains(@class,"card") or contains(@class,"Card")][contains(.,"{film_title}")] | //a[contains(.,"{film_title}")]',
                )
            ).with_(timeout=8).should(be.visible)
