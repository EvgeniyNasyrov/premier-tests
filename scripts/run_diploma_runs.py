...
Каждый прогон — свой Allure-отчёт и своё уведомление в Telegram (по 100% в каждом).
Использование:
...
# Три прогона — по одному на каждый тип
# UI: откат к стабильному набору 11 тестов (как раньше — меньше падений из-за premier.one)
_ui_exclude = (
    "not test_select_film_by_genre and not test_registration_with_valid_email "
...
SUITES = [
    ...
    {"alluredir": "allure-results-ui", "label": "UI", "pytest_args": ["tests/UI/", "-v", "--headless", "-k", _ui_exclude]},
    {"alluredir": "allure-results-mobile", "label": "Mobile", "pytest_args": ["tests/mobile/test_main_mock.py", "-v"]},
]
...
def send_telegram_rich_suite(
    ...
    # В карточке и в подписи — название с типом прогона (API / UI / Mobile)
