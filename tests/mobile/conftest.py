...
def mobile_driver(request):
    ...
    # Диалог «Обновите PREMIER»: нажать только «Обновить»/Update (приложение обновится и зайдёт)
    for text in ("Обновить", "Update"):
        try:
            driver.implicitly_wait(5)
            btn = driver.find_element(AppiumBy.ANDROID_UIAUTOMATOR, f'new UiSelector().text("{text}")')
            btn.click()
            import time
            time.sleep(10)  # дать приложению обновиться и вернуться
            break
        except Exception:
            pass

    # Закрыть онбординг «Discover the Best Apps» — нажать SKIP (иногда несколько слайдов)
    for _ in range(4):
        clicked = False
        for text in ("SKIP", "Пропустить", "Skip"):
            try:
